...
--
-- Servidor: localhost
-- Tiempo de generación: 18-06-2023 a las 12:49:24
...
CREATE TABLE `cliente` (
  ...
  `recomendación_cliente` int(11) DEFAULT NULL
...
INSERT INTO `cliente` (`id`, `nombre`, `dirección postal`, `telefono`, `email`, `fecha de registro`, `recomendación_cliente`) VALUES
(1, 'Darko Morandini', 'Calle X numero 2, 08003. Barcelona', '666666666', 'darkom@gmail.com', '2023-06-09 16:44:24', 1);
...
INSERT INTO `gafas` (`id`, `marca`, `graduación izq`, `graduación der`, `montura`, `color cristal izq`, `color cristal der`, `color montura`, `precio`, `empleado_id`, `cliente_fk`, `marcas__id`) VALUES
(1, 'primera', 0.2, 0.9, 'pasta', 'azul', 'azul', 'azul', 99.99, 1, NULL, 0);
...
ALTER TABLE `cliente`
  ADD PRIMARY KEY (`id`),
  ADD KEY `recomendación_id` (`recomendación_cliente`);
...
ALTER TABLE `Marcas`
  ADD PRIMARY KEY (`id`),
  ADD KEY `proveedor_fk` (`proveedor_id`);
...
--
ALTER TABLE `cliente`
  ADD CONSTRAINT `recomendación_id` FOREIGN KEY (`recomendación_cliente`) REFERENCES `cliente` (`id`);

--
-- Filtros para la tabla `gafas`
--
ALTER TABLE `gafas`
  ADD CONSTRAINT `cliente_id` FOREIGN KEY (`cliente_fk`) REFERENCES `cliente` (`id`),
